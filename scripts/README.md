...
| `budget-monitor.sh` | Shell hook | Claude/Codex `UserPromptSubmit` event | **NO-OP** (context-monitor redesign). It used to emit 50/75/90/95% context advisories, but a UserPromptSubmit hook can't see the model's real context-window size, so the percentage was a guess that over-reported ~5x on 1M models. Context management is now handled by Claude Code (native % + auto-compact) and CCC (statusLine-driven `/compact` vs `/handoff` prompt). Stays wired (drains stdin + exits) so the no-op propagates on update. `CCC_CONTEXT_BUDGET` has no effect. | Yes (P1.6) |
