...
- **Context-budget monitor** — `.harness/scripts/budget-monitor.sh` (UserPromptSubmit). **NO-OP** as of the context-monitor redesign. It used to emit 50/75/90/95% advisories, but a UserPromptSubmit hook can't see the model's real context-window size (only `session_id` + `transcript_path`), so the percentage was a guess that over-reported ~5x on 1M models. Context management now belongs to Claude Code (native % + auto-compact) and CCC (statusLine-driven `/compact` vs `/handoff` prompt at a high real threshold). The hook stays wired so the no-op propagates on update; the script drains stdin and exits. `CCC_CONTEXT_BUDGET` has no effect.
